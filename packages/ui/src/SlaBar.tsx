...
export type SlaBarProps = {
  /** SLA remaining percentage 0–100; null when due date is unknown */
  value: number | null;
...
/**
 * Horizontal SLA health bar with teal / amber / red thresholds, or neutral when unknown.
 */
  ...
  if (value === null) {
    return (
      <div className={`flex items-center ${compact ? 'gap-1' : 'gap-2'}`}>
        ...
        />
        <span
          className={`text-right text-gray-400 ${compact ? 'min-w-[26px] text-[10px]' : 'min-w-[30px] text-[11px]'}`}
        >
          —
        </span>
      </div>
